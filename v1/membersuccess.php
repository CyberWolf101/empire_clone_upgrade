...
<head>
  ...
  <title>Chbluxuryempire - Membership</title>
...
$reed = $_GET['status'];
  ...
  $string=trim($ref);
		    ...
		    if ($reed == "cancelled")
			  {
			  echo "<h2>Payment Cancelled. Please try Again</h2>";
				  } 
			 else 
			  {
			       echo'<h2>WELCOME TO CHBLUXURY EMPIRE MEMBERSHIP!</h2>
          <p>
		  Here is your Membership ID - <span style="font-size:large; text-transform:none; color:#FFC700;">'.$string.' </span><br></p>
<p style="font-size:14px; font-weight:500;">kindly check your email to go through your membership package and also come pick up your memberhsip card at our office<br><span >
<a href="https://chbluxuryempire.com/" style="color:#FFC700;" >Click here to Home</a><p>';


$sql = "SELECT sum(price) from meme where orderid='$string' ";
...
$price=$k;
			 ...
			 $sql = "SELECT * FROM meme where id='$string' ";
             $sql2 = mysqli_query($con,$sql);
		   while($row = mysqli_fetch_array($sql2))
			  {  
...
$html .= "<tr><td  style='color:#FFC700; font-size:14px; font-weight:500;'>" . $name[$key]. "</td><td  style='color:#FFC700; font-size:14px; font-weight:500;'>&#8358;" . $surname[$key]. "</td>
		 ...
		 $sql = "SELECT * FROM member WHERE id='$string'";
                      $sql2 = mysqli_query($con,$sql);
		   while($row = mysqli_fetch_array($sql2))
			  {  
 ...
 $insert = mysqli_query($con,"UPDATE member SET app= 'Yes' where id='$string'") or die ('Could not connect: ' .mysqli_error($con)); 
	...
	$email_to = "$emails";
    $email_subject = "Membership Package!";
             $comments ="
	...
	<p style='color:#FFC700; font-size:30px;'>$mem</p> 
	 
	 </div>
 <p style='color:white;'>Hello Dear Customer,Here is your $mem ID which will be provided to the receptionist to get your membership card.Thank you.</p>
 <p><table border='1px' bordercolor='#000000' cellpadding='10' style='color:#FFFFFF;' width='500px'>
 <tr style='border-bottom:#FFFFFF solid; font-size:14px; font-weight:500;'><td>Membership ID</td>
 <td style='color:#FFC700; font-size:14px; font-weight:500;' >$string</td></tr>
 <tr><td  style='color:#fff; text-align:center;'>From</td><td>To</td></tr>
 <tr><td style='color:#FFC700; font-weight:500;'>$dear </td><td style='color:#FFC700; font-weight:500;'>$end </td></tr>
 <tr><td  style='color:#fff; text-align:center;'>Services</td><td>Price</td></tr>
 $html 
 </table>
