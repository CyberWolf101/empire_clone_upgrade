...
<thead>
	...
	include "connect_to_mysqli.php";
	    ...
	    if((!$sa))
	  {
			 ...
			 $sql = "SELECT DISTINCT id,name from cart where status ='Paid' ORDER BY s DESC";
...
{
	$sql = "SELECT DISTINCT id,name from horder where id='$sa' && status ='Paid' ORDER BY s DESC";
		  $sql2 = mysqli_query($con,$sql);
		$i=1;
while ($row = mysqli_fetch_array($sql2)) {
