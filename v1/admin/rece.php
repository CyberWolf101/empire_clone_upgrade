...
<body>
    			 ...
    			 if (isset($_POST['ordid']))
 {
   ...
   $insert = mysqli_query($con,"UPDATE cart SET app= 'Confirmed' where id='$sa'") or die ('Could not connect: ' .mysqli_error($con)); 
   $insert = mysqli_query($con,"UPDATE cart SET status='Paid' where id='$sa'") or die ('Could not connect: ' .mysqli_error($con)); 
   $insert = mysqli_query($con,"UPDATE cart SET meth= '$sad' where id='$sa'") or die ('Could not connect: ' .mysqli_error($con));  
...
echo $tot;
  ?> naira</p>  
   <p style="text-align:center;"><i>Thank you for your patronage</i></p>
      <a href="dasher.php" style="text-align:center;" class="con">Done</a>
    </div>
