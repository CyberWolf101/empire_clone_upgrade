<?php include "header.php";
 ...
 $del = mysqli_query($con,"DELETE from rental_cart where s='$item_delete'") or die ('Could not connect: ' .mysqli_error($con)); 
						  ...
						  <table class="table table-bordered text-center" style="border-collapse: collapse;">
                           <thead style="background: #FFC700; color: white; border-style: 1px solid #FFC700;">
                            <tr>
                                ...
                                <td style="border-right-style: hidden;">Price(Days) * Quantity</td>
                                <td style="border-right-style: hidden;">Days</td>
                                <td>Total</td>
                            </tr>
                            </thead>
                            <tbody>
...
$sql = "SELECT * from rental_cart where id='$saloon' ";
...
{ echo '
                                ...
                                <td style="vertical-align: middle; border-left-style: hidden;">&#8358;'.$row['itemprice'].' * '.$row['quantity'].'</td>
                                <td width="50" style="vertical-align: middle; border-left-style: hidden;">'.$row['days'].'</td>
                        ...
                        <a href="category.php" style="font-size: 0.8rem; color: rgb(209, 209, 209); font-weight: 600;" class="btn btn-secondary">Update Cart</a></td>
                        </tr>
                       </tfoot>                        
                       </table>
                       </div>
         ...
         $.ajax({
            ...
            success: function(response) {
               if (response === 'success') {
                  ...
                  window.location.href = 'https://chbluxuryempire.com/rental/success?status=completed&tx_ref=<?php echo $saloon; ?>';
   ...
   function updateValues() {
    ...
    $("html, body").animate({
      scrollTop: offsetTop
        ...
        <input type="hidden" name="redirect_url" value="https://chbluxuryempire.com/rental/success.php" />
