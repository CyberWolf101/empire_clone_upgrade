...
//select order
$sql = "SELECT * from rentals where orderid='$order'";
...
values ('$giftcard','$order','$order_amount','$amount_left','$date','processed','Rental')") or die ('Could not connect: ' .mysqli_error($con));
echo "success";
}
...
$submit = mysqli_query($con,"insert into giftcard_history(giftcardno,orderid,amount_deducted,amount_left,date,status,description)
values ('$giftcard','$order','$giftcard_amount','0','$date','processing','Rental')") or die ('Could not connect: ' .mysqli_error($con));
echo "half-success";
}
