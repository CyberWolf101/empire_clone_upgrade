...
$insert = mysqli_query($con,"UPDATE saloon_orders SET gift_amount='$order_amount' where id='$order'") or die ('Could not connect: ' .mysqli_error($con)); 
$insert = mysqli_query($con,"UPDATE saloon_orders SET method='Giftcard' where id='$order'") or die ('Could not connect: ' .mysqli_error($con)); 
...
$submit = mysqli_query($con,"insert into giftcard_history(giftcardno,orderid,amount_deducted,amount_left,date,status,description)
values ('$giftcard','$order','$order_amount','$amount_left','$date','processed','Orishirishi')") or die ('Could not connect: ' .mysqli_error($con));
...
$amount_left=0;
$submit = mysqli_query($con,"insert into giftcard_history(giftcardno,orderid,amount_deducted,amount_left,date,status,description)
values ('$giftcard','$order','$giftcard_amount','$amount_left','$date','processing','Orishirishi')") or die ('Could not connect: ' .mysqli_error($con));
echo "half-success";
}
