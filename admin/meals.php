...
if(isset($_GET['categoryid'])){
    ...
    $del = mysqli_query($con,"DELETE from delta_meals where s='$service_delete'") or die ('Could not connect: ' .mysqli_error($con)); 
    echo "<script>alert('Meal Deleted Successfully!'); window.location.href = 'meals.php';</script>";
...
if(isset($_POST["update_store"]) && !empty($_FILES["file"]["name"])){
    ...
    if(in_array($fileType, $allowTypes)){
        ...
        if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
            ...
            $insert = mysqli_query($con,"UPDATE delta_meals SET picture='".$fileName."' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));  
            if($insert){
...
$insert = mysqli_query($con,"UPDATE delta_meals SET name='$name' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
$insert = mysqli_query($con,"UPDATE delta_meals SET price='$price' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
$insert = mysqli_query($con,"UPDATE delta_meals SET category='$category' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
echo "<script>alert('Meal Updated Successfully!'); window.location.href = 'meals.php';</script>";
}  

?>

             <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Additional Meals</h1>
            <ol class="breadcrumb">
...
<?php include "addmeal.php"; ?>
<p><button onClick="showMeal()"class="btn btn-warning" >Add New Meal</button></p>
...
<input type='submit' name='register' value='Register Meal' class='btn btn-primary w-100' ></form>	
</div></div>
            ...
            <div class="col-lg-12" style="margin-top:2%;">
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Additional Meals</h6>
                </div>
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush text-primary" id="dataTable">
                    ...
                    <tbody>
                        <?php
$sql = "SELECT * from delta_meals  ORDER BY s ASC";
$sql2 = mysqli_query($con,$sql);
while ($row = mysqli_fetch_array($sql2)) {
$category_meal=$row['category'];
...
echo'	<div class="modal fade" id="modal'.$row['s'].'" tabindex="-1">
				...
				<h6 style="color:black;">Edit Meal</h6>
				</div>
