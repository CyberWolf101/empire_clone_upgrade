...
$repair=$_GET['repairid'];

$sql = "SELECT * from repair_center where repair_id='$repair' ORDER BY s DESC";
...
function promptSubmitForm() {
  ...
  if (confirmation) {
    ...
    return false;
             ...
             <h1 class="h5 mb-0 text-black-800" style="text-transform:uppercase;"><?php echo $name; ?><br><span class="badge badge-warning"><?php echo $user_status; ?></span></h1>
             <ol class="breadcrumb">
...
<div class="row" style="margin-bottom:10%;"><div class="col-lg-6">
<p class="prepair"> <?php if($media!=""){ ?><img src="../repairs/<?php echo $media; ?>" class="repairimg"/><?php } ?> <?php if($media2!=""){ ?> <img src="../repairs/<?php echo $media2; ?>" class="repairimg"/></p> <?php } ?>
...
<option value="repair in progress" disabled>Repair in progress </option> 
...
<button type="submit" name="mark" value="Apply Action" class="btn btn-sm btn-warning">Apply Action</button>
</form>';
}
...
else if  ($admin_status !== "superadmin" &&  $user_status =="repair in progress") { 
    echo '<form method="post" id="myForm" onsubmit="return promptSubmitForm();">
<select class="form-control" name="action" id="myFormInput" required>
<option value="" selected> - Select Action -</option>    
<option value="check  in progress" disabled > Accept Request </option> 
...
header('Refresh:1; url=viewrepair.php?repairid='.$repair.''); 
}}}
...
window.location.assign('viewrepair.php?repairid='.$repair.'')</script>";
}



//update and save 
$info= mysqli_real_escape_string($con, $inform);
$submit = mysqli_query($con,"insert into repair_failures (`repair_id`, `message`) VALUES ('$repair', '$info')") or die ('Could not connect: ' .mysqli_error($con));  
}
                ...
                <div class="modal-body">
                  <form id="form" name="form" action="" method="post" enctype="multipart/form-data" > 
	                 ...
	                 <p><label>Add an image if required</label><input type="file"  name="product"  class="form-control" /></p>
					  ...
					  <div class="modal-footer">
					  <input id="submit" name="message" class="btn btn-sm btn-warning shadow-sm" type="submit" value="Send Message"></form>
                    </div>
                  </div>
                </div></div>
