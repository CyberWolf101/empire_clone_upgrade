...
if (isset($_COOKIE['orderID'])) {
        ...
        if ($statuspay == "") {
                ...
                values ('$ran','','','',0,'','','','$date',0,0,0,0,0,0,'store','refreshments','0','',0,0)") or die('Could not connect: ' . mysqli_error($con));

                //setcookie
                setcookie("orderID", $ran, time() + (10 * 365 * 24 * 60 * 60));
        ...
        $submit = mysqli_query($con, "insert into saloon_orders(id,name,email,phone,bookingtype,method,pay_status,status,date,saloonkit,total_amount,card_amount,cash_amount,transfer_amount,pos_amount,type,section, giftcard, gift_amount, preorder, preorder_date)
        values ('$ran','','','',0,'','','','$date','0','0','0','0','0','0','store','refreshments', '0','0','0', '')") or die('Could not connect: ' . mysqli_error($con));
