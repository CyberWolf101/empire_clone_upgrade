...
$fileType = pathinfo($targetFilePath,PATHINFO_EXTENSION);
 ...
 $targetDir ="../repairs/";
$fileName2 = basename($_FILES["invoice"]["name"]);
$targetFilePath = $targetDir . $fileName2;
$fileType = pathinfo($targetFilePath,PATHINFO_EXTENSION);

 if(isset($_POST["repair"]) && !empty($_FILES["invoice"]["name"])){
    // Allow certain file formats
    $allowTypes = array('jpg','png','jpeg','gif','pdf');
    if(in_array($fileType, $allowTypes)){
     // Upload file to server
     if(move_uploaded_file($_FILES["product"]["tmp_name"], $targetFilePath)){
     // Insert image file name into database
     $insert = mysqli_query($con,"INSERT into images (file_name, uploaded_on) VALUES ('".$fileName2."', NOW())");
 }}}

$date=date('Y-m-d');
$code=substr(mt_rand(), 0, 4);
$id='RP'.$code;
$submit = mysqli_query($con,"insert into repair_center(repair_id,name, contact,email, item, description,purchase,duration, picture,invoice, date, status,repairhistory) 
values ('$id','$name', '$phone', '$email','$item', '$damage','$purchase','$duration', '".$fileName."', '".$fileName2."', '$date', 'pending','no')") or die ('Could not connect: ' .mysqli_error($con));
...
$mailsent=mail($email_to, $email_subject,$email_message,$header);
  if($mailsent ==true){
    ...
    $email_messages ="
     ...
     <pstyle='color:#FFFFFF;'>Item is a $item <br>The tracking code is <span style='color:#FFC700;'>$id</span></p>
...
header('Refresh:3; url=startrepair.php');
