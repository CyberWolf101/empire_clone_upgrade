...
if(isset($_GET['categoryid'])){
    ...
    $del = mysqli_query($con,"DELETE from rental_subcategories where s='$service_delete'") or die ('Could not connect: ' .mysqli_error($con)); 
    echo "<script>alert('Subcategory Deleted Successfully!'); window.location.href = 'rental_subcategory.php';</script>";
    ...
    if(in_array($fileType, $allowTypes)){
        ...
        if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
            ...
            $insert = mysqli_query($con,"INSERT into images (file_name,input,uploaded_on) VALUES ('".$fileName."','staff', NOW())");
             $insert = mysqli_query($con,"UPDATE rental_subcategories SET picture='".$fileName."' where s='$id'") or die ('Could not connect: ' .mysqli_error($con)); 
            if($insert){
...
$insert = mysqli_query($con,"UPDATE rental_subcategories SET name='$name' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
$insert = mysqli_query($con,"UPDATE rental_subcategories SET description='$des' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
echo "<script>alert('Subcategory Updated Successfully!'); window.location.href = 'rental_subcategory.php';</script>";
}  

?>

             <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h5 mb-0 text-gray-800">Rental Subcategories</h1>
            <ol class="breadcrumb">
...
<?php include "addrental.php"; ?>
<p><button onClick="showAri()"class="btn btn-warning w-100" >Add New Subcategory</button></p>	
              ...
              <div class="card mb-4">
                ...
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush text-primary" id="dataTable">
                    ...
                    <tbody>
                        <?php
$sql = "SELECT * from rental_subcategories  ORDER BY s ASC";
...
echo'	<div class="modal fade" id="modal'.$row['s'].'" tabindex="-1">
				...
				<h6 style="color:black;">Edit Subcategory</h6>
				</div>
