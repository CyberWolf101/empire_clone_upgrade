...
if(isset($_GET['categoryid'])){
    ...
    $del = mysqli_query($con,"DELETE from gift_packages where s='$service_delete'") or die ('Could not connect: ' .mysqli_error($con)); 
    echo "<script>alert('Package Deleted Successfully!'); window.location.href = 'createpackages.php';</script>";
...
if(isset($_POST["update_store"]) && !empty($_FILES["file"]["name"])){
    ...
    if(in_array($fileType, $allowTypes)){
        ...
        if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
            ...
            $insert = mysqli_query($con,"UPDATE gift_packages SET picture='".$fileName."' where s='$id'") or die ('Could not connect: ' .mysqli_error($con)); 
            if($insert){
...
$insert = mysqli_query($con,"UPDATE gift_packages SET name='$name' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
$insert = mysqli_query($con,"UPDATE gift_packages SET price='$price' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
$insert = mysqli_query($con,"UPDATE gift_packages SET services='$commaSeparatedSections' where s='$id'") or die ('Could not connect: ' .mysqli_error($con));
echo "<script>alert('Package Details Updated Successfully!'); window.location.href = 'createpackages.php';</script>";
}  

?>

             <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h5 mb-0 text-gray-800">Gift Packages</h1>
            <ol class="breadcrumb">
...
<?php include "addvoucher.php"; ?>
<p><button onClick="showMeal()"class="btn btn-warning" >Add New Package</button></p>
...
<div class="form-group"><label>Add Services</label>
...
<input type='submit' name='register' value='Register Package' class='btn btn-primary w-100' ></form>	
</div></div>
            ...
            <!-- Datatables -->
            <div class="col-lg-12" style="margin-top:2%; font-size:14px; font-weight:700;">
              <div class="card mb-4">
                ...
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush text-primary" id="dataTable">
                    ...
                    <tbody>
                        <?php
$sql = "SELECT * from gift_packages ORDER BY s ASC";
$sql2 = mysqli_query($con,$sql);
while ($row = mysqli_fetch_array($sql2)) {
    ...
    while ($rom = mysqli_fetch_array($sq2)) {
        ...
        $categoryNames[] = $categoryname;
                    ...
                    echo implode(', ', $categoryNames) ;
                         ...
                         <td><form action='' method='get' onsubmit='return confirm(\"Are you sure you want to delete this package (".$row['name'].")?\");'>
...
echo'	<div class="modal fade" id="modal'.$row['s'].'" tabindex="-1">
				...
				<h6 style="color:black;">Edit Package</h6>
				</div>
...
$sqb = "SELECT id, name FROM services";
