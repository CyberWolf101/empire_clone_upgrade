...
$sql = "SELECT * from members where cardno='$string' ";
...
$insert = mysqli_query($con,"UPDATE members SET paystatus='paid' where cardno='$card'") or die ('Could not connect: ' .mysqli_error($con)); 
$insert = mysqli_query($con,"UPDATE members SET status='valid' where cardno='$card'") or die ('Could not connect: ' .mysqli_error($con)); 
$startdate=date("Y-m-d");
$enddate = date('Y-m-d', strtotime("$startdate +$day days"));
$insert = mysqli_query($con,"UPDATE members SET start_date='$startdate' where cardno='$card'") or die ('Could not connect: ' .mysqli_error($con)); 
$insert = mysqli_query($con,"UPDATE members SET end_date='$enddate' where cardno='$card'") or die ('Could not connect: ' .mysqli_error($con)); 
...
if (mysqli_num_rows($sql2) > 0) {
    ...
    <tr style='border-bottom:#FFFFFF solid;'><td>Member ID</td><td style='color:#FFC700;' colspan='3' >$card</td></tr>
     <tr><td style='color:#fff; text-align:center;'>Your Packages</td><td>Price</td><td>Duration(month)</td><td>Total</td></tr>
     ";
...
$total_all=$row["total_amount"];
	...
	$email_from=$sitemail;					 
	$email_to = $email;
    $email_subject = "Membership Package! - $sitename";
    $email_message ="
	 ...
	 <font color='#FFFFFF' style='float:right; font-size:15px; padding-right:6px; text-align:right; margin-top:13px;'>Date: $startdate</font></p>
	
	
		<p style='color:#FFC700; font-size:30px;'>$card</p>
		<h5>MEMBERSHIP DETAILS</h5>
		<p style='color:white;'>$name<br>$email<br>$phone<br>$type<br>Membership Expires: $enddate</p>
	    <p style='color:white;'>Hello Dear Customer,Here is your membership ID which will be provided to the receptionist to get your membership card.Here are your purchased packages</p>
         $html
...
////////////////////////////////////////////End mail Function////////////////////////////////////////////////////////////// 
	...
	$email_from=$sitemail;					 
	$email_to = $sitemail;
    $email_subject ="New Member Registeration - $sitename";
    $email_message ="
     ...
     <p style='color:#fff !important;'>Hello Dear Admin,There has been a new member registeration.Login to your dashboard to view</p><br><br>
...
<p>Here is your Member ID - <span style="font-size:large; text-transform:none; color:#FFC700;">'.$ref .'</span><br></p>
<p style="font-size:17px; font-weight:700;">Your membership package have been sent to you.
