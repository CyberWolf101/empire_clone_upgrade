...
// Redirect if user details are already set
if (isset($_SESSION['username']) && $_SESSION['username'] != "") {
    header("Location: cart.php");
    exit;
}

// Initialize $saloon if not set
if (!isset($saloon) || empty($saloon)) {
    $saloon = uniqid('ORDER_');
    setcookie("foodID", $saloon, time() + 3600, "/", "", true, true);
    $query = "INSERT INTO saloon_orders (id, status, pay_status, payment_confirmed) VALUES ('$saloon', 'pending', 'pending', 0)";
